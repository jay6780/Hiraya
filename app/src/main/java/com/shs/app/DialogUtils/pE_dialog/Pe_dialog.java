package com.shs.app.DialogUtils.pE_dialog;
...
import com.shs.app.Activity.Admin.checkList.pE_checklist;
import com.shs.app.Class.Student.Students;
import com.shs.app.R;

import java.util.List;

public class Pe_dialog {
    private pE_checklist activity;
    private List<Students> studentList;

    private String[][] studentData;

    public Pe_dialog(pE_checklist activity,  List<Students> studentList, String[][] studentData) {
    ...
    private void deleteDataForStudent(String studentId) {
        DatabaseReference grade2Ref = FirebaseDatabase.getInstance().getReference().child("Grade2").child(studentId).child("Physical_Education_quarterly_assessment");
        DatabaseReference writtenWorksRef = FirebaseDatabase.getInstance().getReference().child("written_works").child(studentId).child("Physical_Education_written_works");
        DatabaseReference gradeRef = FirebaseDatabase.getInstance().getReference().child("Grade").child(studentId).child("Physical_Education_performanceTask");
    ...
    public void editScores(String studentId, String studentName, String studentImage, int rowIndex) {
        ...
        updatedBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                gradeRef.child("Physical_Education_performanceTask").setValue(performanceTaskScore);
                DatabaseReference writtenWorksRef = FirebaseDatabase.getInstance().getReference().child("written_works").child(studentId);
                writtenWorksRef.child("Physical_Education_written_works").setValue(writtenWorksScore);
                DatabaseReference grade2Ref = FirebaseDatabase.getInstance().getReference().child("Grade2").child(studentId);
                grade2Ref.child("Physical_Education_quarterly_assessment").setValue(quarterlyAssessmentScore);
